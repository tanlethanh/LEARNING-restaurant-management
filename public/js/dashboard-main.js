...
function tableOnClick(event) {
    ...
    let customer = findCusById(customersData, chosenCustomer? chosenCustomer.id: "")
    ...
    else if (customer != null && event.currentTarget.classList.contains("popup")) {
        const title = `Bạn có muốn gán <strong>Bàn số ${table.tableNumber}</strong> 
                cho khách hàng số <strong>${customer.ordinamNumber}</strong>?`
        createYesNoModal(title, alert("oke"));
...
const listNewCustomer = document.getElementsByClassName("newCustomer-unassigned");
...
function customerOnClick(event){
    ...
    if (chosenCustomer != null &&
        chosenCustomer.id == event.currentTarget.id
    ) {
...
function popUpCusMatchedTables(currentTarget) {
    ...
    // Pop up all tables available for this reservation
    tablesData.map(table => {
        if (table.numberOfSeats >= curCustomer.numberOfSeats
            && table.state == "FREE"
        ) {
            const tableElement = document.getElementById(table.id)
            if (!tableElement.classList.contains("popup")) {
